...
namespace BLabTexasHoldEmProject
{
    public class BBPreFlopPhaseAIController : MonoBehaviour
    {
        ...
        public void executeSmallBlindBet()
        {
            ...
            {
#if USE_PHOTON_BB
                BBGC_M._BBGlobalDefinitions.currentActivePlayer =
                    BBGC_M._BBGlobalDefinitions.smallBlindPlayerId;
                BBGC_M.playerDataList[
                    BBGC_M._BBGlobalDefinitions.currentActivePlayer
                ].gamePhaseDetail = BBGlobalDefinitions.GamePhaseDetail.SmallBlind;
                if (BBStaticData.debugPreFlopController)
                    Debug.Log(
                        "[BBPreFlopPhaseAIController][executeSmallBlindBet] smallBlindPlayerId : "
                            + BBGC_M._BBGlobalDefinitions.smallBlindPlayerId
                    );
                GetComponent<BBGuiInterfaceMultiplayer>().simulateOnClickButton("BET_25");
                GetComponent<BBGuiInterfaceMultiplayer>()
                    .setPlayerBetType(
                        BBGC_M._BBGlobalDefinitions.currentActivePlayer,
                        "SmallBlind"
                    );
            ...
            {
                BBGC._BBGlobalDefinitions.currentActivePlayer =
                    BBGC._BBGlobalDefinitions.smallBlindPlayerId;
                if (BBStaticData.debugPreFlopController)
                    Debug.Log(
                        "[BBPreFlopPhaseAIController][executeSmallBlindBet] smallBlindPlayerId : "
                            + BBGC._BBGlobalDefinitions.smallBlindPlayerId
                ...
                GetComponent<BBGuiInterface>()
                    .setPlayerBetType(BBGC._BBGlobalDefinitions.currentActivePlayer, "Small");
            }
        }
...
#if USE_PHOTON_BB
                BBGC_M._BBGlobalDefinitions.currentActivePlayer =
                    BBGC_M._BBGlobalDefinitions.bigBlindPlayerId;
                BBGC_M.playerDataList[
                    BBGC_M._BBGlobalDefinitions.currentActivePlayer
                ].gamePhaseDetail = BBGlobalDefinitions.GamePhaseDetail.BigBlind;
                if (BBStaticData.debugPreFlopController)
                    Debug.Log(
                        "[BBPreFlopPhaseAIController][executeSmallBlindBet] bigBlindPlayerId : "
                            + BBGC_M._BBGlobalDefinitions.bigBlindPlayerId
                    );
                GetComponent<BBGuiInterfaceMultiplayer>().simulateOnClickButton("BET_50");
                GetComponent<BBGuiInterfaceMultiplayer>()
                    .setPlayerBetType(BBGC_M._BBGlobalDefinitions.currentActivePlayer, "BigBlind");
#endif
            }
            else
            {
                BBGC._BBGlobalDefinitions.currentActivePlayer =
                    BBGC._BBGlobalDefinitions.bigBlindPlayerId;
                if (BBStaticData.debugPreFlopController)
                    Debug.Log(
                        "[BBPreFlopPhaseAIController][executeSmallBlindBet] bigBlindPlayerId : "
                            + BBGC._BBGlobalDefinitions.bigBlindPlayerId
                ...
                GetComponent<BBGuiInterface>()
                    .setPlayerBetType(BBGC._BBGlobalDefinitions.currentActivePlayer, "Big");
            }
        }
            ...
            if (isMultiplayer)
            {
                ...
                if (BBGC_M.playerDataList[playerCounter].playerName == PhotonNetwork.player.name)
                {
                    GetComponent<BBSoundController>()
                        .playClip(GetComponent<BBSoundController>().genBip);
                    GetComponent<BBGuiInterfaceMultiplayer>()
                    ...
                    yield return new WaitUntil(
                        () => BBGC_M.waitingForManualButtonResponce == false
                    );
                }
#endif
            }
            else
            {
                for (int x = 0; x < BBGC.playerDataList.Count; x++)
                {
                    if (BBStaticData.debugPreFlopController)
                        Debug.Log(
                            playerCounter
                                + ")+++++++++++++++++++ [BBPreFlopPhaseAIController] after [executePreFlopFirstBetRound] playersStateIsOutDuringOpenGame : "
                                + BBGC._BBGlobalDefinitions.playersStateIsOutDuringOpenGame[
                                    playerCounter
                                ]
                        );
                    ...
                    else
                    {
                        ...
                        if (playerCounter == BBGC._BBGlobalDefinitions.localPlayer)
                        {
                            ...
                            yield return new WaitUntil(
                                () => BBGC.waitingForManualButtonResponce == false
                            );
                        }
                        else
                        {
                            ...
                            if (BBStaticData.debugPreFlopController)
                                Debug.Log(
                                    "[BBPreFlopPhaseAIController] after [executePhaseOnAIPlayer] playerCounter : "
                                        + playerCounter
                                        + " talker : "
                                        + BBGC._BBGlobalDefinitions.playerToTalk
                                );
                        }
                    }
                    playerCounter++;
                ...
                if (BBStaticData.debugPreFlopController)
                    Debug.Log(
                        "#################### DONE READY FOR LAST CECK ##################[BBPreFlopPhaseAIController] after [executePhaseOnAIPlayer] playerCounter : "
                            + playerCounter
                            + " talker : "
                            + BBGC._BBGlobalDefinitions.playerToTalk
                    );
        ...
        IEnumerator executePreFlopLastBetChecking()
        {
            ...
            GetComponent<BBGuiInterface>().TextGamePhase.text =
                "Phase : " + BBGC._BBGlobalDefinitions.gamePhaseDetail.ToString();

            float maxPlayerVal = 0;
            foreach (BBPlayerData pd in BBGC.playerDataList)
            {
                if (pd.currentMoneyOnTable >= maxPlayerVal)
                    maxPlayerVal = pd.currentMoneyOnTable;
            }

            if (BBStaticData.debugPreFlopController)
                Debug.Log(
                    "[BBPreFlopPhaseAIController][executePreFlopLastBetChecking] phase : "
                        + BBGC._BBGlobalDefinitions.gamePhaseDetail
                        + " maxPlayerVal : "
                        + maxPlayerVal
                );
            ...
            for (int x = 0; x < BBGC.playerDataList.Count; x++)
            {
                if (BBStaticData.debugPreFlopController)
                    Debug.Log(
                        "[BBPreFlopPhaseAIController][executePreFlopLastBetChecking] >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> : "
                            + BBGC.playerDataList[playerCounter].isOutOfGame
                            + " : "
                            + BBGC._BBGlobalDefinitions.playersStateIsOutDuringOpenGame[
                                playerCounter
                            ]
                ...
                if (
                    !(BBGC.playerDataList[playerCounter].isOutOfGame)
                    && !(BBGC._BBGlobalDefinitions.playersStateIsOutDuringOpenGame[playerCounter])
                )
                {
                    GetComponent<BBGuiInterface>().activatePlayerToTalk(playerCounter);
                    ...
                    float diff =
                        maxPlayerVal - BBGC.playerDataList[playerCounter].currentMoneyOnTable;
                    if (BBStaticData.debugPreFlopController)
                        Debug.Log(
                            "[BBPreFlopPhaseAIController][executePreFlopLastBetChecking] currentActivedealer diff : "
                                + diff
                                + " player : "
                                + playerCounter
                        );
                    ...
                    if (playerCounter == BBGC._BBGlobalDefinitions.localPlayer)
                    {
                        ...
                        yield return StartCoroutine(
                            GetComponent<BBMoveingObjectsController>()
                                .moveChip(diff, playerCounter, null)
                        );
                    }
                    else
                    {
        ...
        {
            int coeffvalue = BBStaticData.getCardsPoint(
                BBGC.playerDataList[playerID].card_1_Value,
                BBGC.playerDataList[playerID].card_2_Value
            );
            ...
            {
                if (
                    GetComponent<BBTestSimulationController>().simulateAiPhasePlayers_FirstBettingRound[
                        playerID
                    ] == BBGlobalDefinitions.GamePhaseDetail.FirstBettingRound
                )
                {
                    switch (
                        GetComponent<BBTestSimulationController>().simulateAiCommandPlayers_FirstBettingRound[
                            playerID
                        ]
                    ) {
            ...
            if (BBStaticData.debugPreFlopController)
                Debug.Log(
                    "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*********executePhaseOnAIPlayer**************BBPreFlopPhaseAIController coeffvalue : "
                        + coeffvalue
                        + " playerPos : "
                        + playerID
                        + " phase : "
        ...
        IEnumerator ie_RPCexecuteFold(int playerID)
        {
            ...
            yield return StartCoroutine(
                GetComponent<BBMoveingObjectsController>()
                    .moveCard(
                        BBGC_M.playerDataList[playerID].transform_card_1.gameObject,
                        BBGC_M.cardsDiscardPosition
                    )
            );
            yield return StartCoroutine(
                GetComponent<BBMoveingObjectsController>()
                    .moveCard(
                        BBGC_M.playerDataList[playerID].transform_card_2.gameObject,
                        BBGC_M.cardsDiscardPosition
                    )
            );
        ...
        public IEnumerator executeFold(int playerID)
        {
            ...
            if (isMultiplayer)
            {
                ...
                GetComponent<PhotonView>()
                    .RPC("RPCexecuteFold", PhotonTargets.AllViaServer, playerID);
#endif
            }
            else
            {
                ...
                yield return StartCoroutine(
                    GetComponent<BBMoveingObjectsController>()
                        .moveCard(
                            BBGC.playerDataList[playerID].transform_card_1.gameObject,
                            BBGC.cardsDiscardPosition
                        )
                );
                yield return StartCoroutine(
                    GetComponent<BBMoveingObjectsController>()
                        .moveCard(
                            BBGC.playerDataList[playerID].transform_card_2.gameObject,
                            BBGC.cardsDiscardPosition
                        )
                );
        ...
        {
            StartCoroutine(
                GetComponent<BBMoveingObjectsController>().moveChip(toBet, playerID, null)
            );
        }
        ...
        public IEnumerator executeFirstBetting(int playerID)
        {
            ...
            GetComponent<PhotonView>()
                .RPC("RPCmoveChip", PhotonTargets.All, toBet, playerID);
            yield break;
        }
#endif
        ...
        public IEnumerator executeCall(int playerID)
        {
            ...
            if (isMultiplayer)
            {
#if USE_PHOTON_BB
                if (
                    BBGC_M._BBGlobalDefinitions.gamePhaseDetail
                    == BBGlobalDefinitions.GamePhaseDetail.FirstBettingRound
                )
                {
                ...
                {
                    float[] valList = new float[BBGC_M.playerDataList.Count];
                    for (int x = 0; x < valList.Length; x++)
                        valList[x] = BBGC_M.playerDataList[x].currentMoneyOnTable;
                    ...
                    GetComponent<BBGuiInterfaceMultiplayer>()
                        .setPlayerMoneyValue(tmpToCover, playerID);
                }
            ...
            else
            {
                ...
                {
                    float[] valList = new float[BBGC.playerDataList.Count];
                    for (int x = 0; x < valList.Length; x++)
                        valList[x] = BBGC.playerDataList[x].currentMoneyOnTable;
                    float valueToCover = Mathf.Max(valList);
                    float tmpVal = BBGC.playerDataList[playerID].currentMoneyOnTable;
                    float tmpToCover = valueToCover - tmpVal;
                    Debug.Log(
                        "[TOSAVEONFILE] AI executeCall player : "
                            + playerID
                            + " # "
                            + " valueToCover : "
                            + valueToCover
                            ...
                            + tmpToCover
                            + " # gamePhaseDetail : "
                            + BBGC._BBGlobalDefinitions.gamePhaseDetail.ToString()
                    );
        ...
        IEnumerator ie_RPCexecuteRaise(int playerID)
        {
            ...
            GetComponent<BBGuiInterfaceMultiplayer>().TextGameRAISECounter.text =
                "Round Raise : " + BBGC_M._BBGlobalDefinitions.roundRaiseCounter.ToString();
            float raiseBetVal = 0;
            switch (BBGC_M._BBGlobalDefinitions.roundRaiseCounter)
            {
                case 1:
                    raiseBetVal = BBGC_M._BBGlobalDefinitions.smallBlindValue * 2;
                    break;
                case 2:
                    raiseBetVal = BBGC_M._BBGlobalDefinitions.smallBlindValue * 3;
                    break;
                case 3:
                    raiseBetVal = BBGC_M._BBGlobalDefinitions.smallBlindValue * 4;
                    break;
                default:
                    raiseBetVal = BBGC_M._BBGlobalDefinitions.smallBlindValue * 4;
                    break;
            ...
            if (BBStaticData.debugPreFlopController)
                Debug.Log(
                    "buttonGameRAISE --------->>> raiseBetVal : "
                        + raiseBetVal
                        + " raiseCounter : "
                        + BBGC_M._BBGlobalDefinitions.roundRaiseCounter
                        + " smallBlindValue : "
                        + BBGC_M._BBGlobalDefinitions.smallBlindValue
                        + " talker : "
                        + BBGC_M._BBGlobalDefinitions.playerToTalk
                );
            ...
            GetComponent<BBGuiInterfaceMultiplayer>()
                .setPlayerBetType(playerID, "Raise");
        }
#endif
...
#if USE_PHOTON_BB
                GetComponent<PhotonView>()
                    .RPC("RPCexecuteRaise", PhotonTargets.AllViaServer, playerID);
#endif
            }
            else
            {
                BBGC._BBGlobalDefinitions.roundRaiseCounter++;
                GetComponent<BBGuiInterface>().TextGameRAISECounter.text =
                    "Round Raise : " + BBGC._BBGlobalDefinitions.roundRaiseCounter.ToString();
                float raiseBetVal = 0;

                switch (BBGC._BBGlobalDefinitions.roundRaiseCounter)
                {
                    case 1:
                        raiseBetVal = BBGC._BBGlobalDefinitions.smallBlindValue * 2;
                        break;
                    case 2:
                        raiseBetVal = BBGC._BBGlobalDefinitions.smallBlindValue * 3;
                        break;
                    case 3:
                        raiseBetVal = BBGC._BBGlobalDefinitions.smallBlindValue * 4;
                        break;
                    default:
                        raiseBetVal = BBGC._BBGlobalDefinitions.smallBlindValue * 4;
                        break;
                ...
                if (BBStaticData.debugPreFlopController)
                    Debug.Log(
                        "buttonGameRAISE --------->>> raiseBetVal : "
                            ...
                            + BBGC._BBGlobalDefinitions.roundRaiseCounter
                            + " smallBlindValue : "
                            + BBGC._BBGlobalDefinitions.smallBlindValue
                            + " talker : "
                            + BBGC._BBGlobalDefinitions.playerToTalk
                    );
