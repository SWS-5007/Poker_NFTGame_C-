...
namespace BLabTexasHoldEmProject
{
    public class BBMainMenuController : MonoBehaviour
    {
        ...
        IEnumerator Start()
        {
            ...
            else
            {
                PlayerPrefs.SetFloat(
                    "MPGeneralPlayerMoney",
                    BBStaticVariable.baseStartingMoneyForPlayer
                );
            }

#else
            iap =
                Resources.Load("InAppConfigScriptableAsset")
                as BLab.GetCoinsSystem.InAppConfigScriptableAsset;
            ...
            else
            {
                PlayerPrefs.SetFloat(
                    "MPGeneralPlayerMoney",
                    iap.gameMoneysettings.playerInitialMoney
                );
            }
#endif
        ...
        {
            string nick =
                PanelgetNickname.transform
                    .Find("InputFieldGetNickName")
        ...
        bool checkCanPlayWithMyMoney(bool singlePlayerGame)
        {
            ...
            if (singlePlayerGame)
            {
                if (currentMoney < _BBGlobalDefinitions.stackMoneyAtStart)
                {
                    ...
                    PanelNotEnoughMoney.transform
                        .Find("TextMinMoneyValue")
                        .GetComponent<Text>().text = BBStaticData.getMoneyValue(
                        _BBGlobalDefinitions.stackMoneyAtStart
                    );
                    PanelNotEnoughMoney.transform
                        .Find("TextPlayerMoneyValue")
                        .GetComponent<Text>().text = BBStaticData.getMoneyValue(currentMoney);
                }
                else
                {
                    tmpRet = true;
                ...
                Debug.Log(
                    "[checkCanPlayWithMyMoney] currentMoney : "
                        + currentMoney
                        + " single stackMoneyAtStart : "
                        + _BBGlobalDefinitions.stackMoneyAtStart
                );
            }
            else
            {
                if (currentMoney < BBStaticVariable.gameLimitedStackValue)
                {
                    ...
                    PanelNotEnoughMoney.transform
                        .Find("TextMinMoneyValue")
                        .GetComponent<Text>().text = BBStaticData.getMoneyValue(
                        BBStaticVariable.gameLimitedStackValue
                    );
                    PanelNotEnoughMoney.transform
                        .Find("TextPlayerMoneyValue")
                        .GetComponent<Text>().text = BBStaticData.getMoneyValue(currentMoney);
                ...
                Debug.Log(
                    "[checkCanPlayWithMyMoney] currentMoney : "
                        + currentMoney
                        + " multi BBStaticVariable.gameLimitedStackValue : "
                        + BBStaticVariable.gameLimitedStackValue
                );
            }
