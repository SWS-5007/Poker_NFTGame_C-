...
{
    public class BBUIButtonMessageForUUI
        : MonoBehaviour,
          IPointerDownHandler,
          IPointerUpHandler,
          IPointerClickHandler
    {
        ...
        {
            if (useEvent == _EventType.ButtDown)
                clickCall();
        }

        public void OnPointerUp(PointerEventData eventData)
        {
            if (useEvent == _EventType.buttUP)
                clickCall();
        }

        public void OnPointerClick(PointerEventData eventData)
        {
            if (useEvent == _EventType.click)
                clickCall();
        }
        ...
        public void clickCall()
        {
            ...
            {
                if (includeChildren)
                    toSendGameObject.BroadcastMessage(
                        functionToCall,
                        gameObject,
                        SendMessageOptions.DontRequireReceiver
                    );
                else
                    toSendGameObject.SendMessage(
                        functionToCall,
                        gameObject,
                        SendMessageOptions.DontRequireReceiver
                    );
