    ...
    public class BBMoveingObjectsController : MonoBehaviour
    {
        ...
        public IEnumerator removeAllCards()
        {
            ...
            GetComponent<BBSoundController>()
                .playClip(GetComponent<BBSoundController>().cleanCards);
        ...
        public IEnumerator removeAllChips()
        {
            ...
            GetComponent<BBSoundController>()
                .playClip(GetComponent<BBSoundController>().cleanChips);
            ...
            if (moveMultiObjectMoveList)
            {
                ...
                for (int x = 0; x < multiObjectMoveList.Count; x++)
                {
                    if (multiObjectMoveList[x].destination == null)
                    {
                        ...
                        multiObjectMoveList[x].toMove.position = Vector3.MoveTowards(
                            multiObjectMoveList[x].toMove.position,
                            _to,
                            step
                        );
                        Debug.Log(
                            "moveMultiObjectMoveList-------moveing-------->>"
                                + multiObjectMoveList[x].toMove.name
                                + " : "
                                + _to
                                + " : "
                                + multiObjectMoveList[x].toMove.position
                    ...
                    else
                    {
                        multiObjectMoveList[x].toMove.position = Vector3.MoveTowards(
                            multiObjectMoveList[x].toMove.position,
                            multiObjectMoveList[x].destination.position,
                            step
                        );
                ...
                if (notStillMove < 1)
                {
                    ...
                    else
                    {
                        ...
                        )
                        {
                            ...
                            GetComponent<BBGameController>()._BBGlobalDefinitions.gamePhaseDetail
                            == BBGlobalDefinitions.GamePhaseDetail.BigBlind
                        )
                        {
                            ...
                            GetComponent<BBGameController>()._BBGlobalDefinitions.gamePhaseDetail
                            == BBGlobalDefinitions.GamePhaseDetail.GiveingCards
                        ) { }
                    }
        ...
        public IEnumerator moveChipFromTo(float val, Vector3 _from, Vector3 _to)
        {
            ...
            else
            {
                splitted =
                    (int)val
                    / (int)GetComponent<BBGameController>()._BBGlobalDefinitions.limitedLow;
            }
            ...
            if (splitted == 1)
            {
                ...
                else
                {
                    yield return StartCoroutine(
                        createAndMoveChipFromTo(
                            GetComponent<BBGameController>()._BBGlobalDefinitions.smallBlindValue,
                            chipGREEN,
                            _from,
                            _to
                        )
            ...
            else if (splitted == 2)
            {
                ...
                else
                {
                    yield return StartCoroutine(
                        createAndMoveChipFromTo(
                            GetComponent<BBGameController>()._BBGlobalDefinitions.bigBlindValue,
                            chipBLUE,
                            _from,
                            _to
                        )
                ...
                if (val >= 500)
                {
                    ...
                    else
                    {
                        if (tmpVal >= 100)
                        {
                            ...
                            tmpVal -= 100;
                            yield return StartCoroutine(
                                createAndMoveChipFromTo(100, chipBLACK, _from, _to)
                            );
                            if (tmpVal >= 100)
                            {
                                goto RepFor100;
                            ...
                            else
                            {
                                if (tmpVal >= 50)
                                {
                                    ...
                                    tmpVal -= 50;
                                    yield return StartCoroutine(
                                        createAndMoveChipFromTo(50, chipBLACK, _from, _to)
                                    );
                                    if (tmpVal >= 50)
                                    {
                                        goto RepFor50;
                                    ...
                                    {
                                        yield return StartCoroutine(
                                            createAndMoveChipFromTo(25, chipGREEN, _from, _to)
                                        );
                                    }
                                }
                            }
                ...
                else if (val >= 100)
                {
                    ...
                    else
                    {
                        ...
                        if (tmpVal >= 50)
                        {
                            tmpVal -= 50;
                            yield return StartCoroutine(
                                createAndMoveChipFromTo(50, chipBLUE, _from, _to)
                            );
                            if (tmpVal >= 50)
                            {
                                goto RepFor50_2;
        ...
        public IEnumerator moveChip(float val, int playerID, Transform position)
        {
            ...
            {
                if (
                    GetComponent<BBGameController>()._BBGlobalDefinitions.playersStateIsOutDuringOpenGame[
                        playerID
                    ]
                )
                {
            ...
            else
            {
                ...
                if (val >= 500)
                {
                    ...
                    tmpVal -= 500;
                    yield return StartCoroutine(
                        createAndMoveChip(500, chipRED, playerID, position)
                    );
                    if (tmpVal >= 500)
                    {
                        goto RepFor500;
                    ...
                    else
                    {
                        if (tmpVal >= 100)
                        {
                            ...
                            tmpVal -= 100;
                            yield return StartCoroutine(
                                createAndMoveChip(100, chipBLACK, playerID, position)
                            );
                            if (tmpVal >= 100)
                            {
                                goto RepFor100;
                            ...
                            else
                            {
                                if (tmpVal >= 50)
                                {
                                    ...
                                    tmpVal -= 50;
                                    yield return StartCoroutine(
                                        createAndMoveChip(50, chipBLACK, playerID, position)
                                    );
                                    if (tmpVal >= 50)
                                    {
                                        goto RepFor50;
                                    ...
                                    {
                                        yield return StartCoroutine(
                                            createAndMoveChip(25, chipGREEN, playerID, position)
                                        );
                                    }
                                }
                            }
                ...
                else if (val >= 100)
                {
                    ...
                    else
                    {
                        ...
                        if (tmpVal >= 50)
                        {
                            tmpVal -= 50;
                            yield return StartCoroutine(
                                createAndMoveChip(50, chipBLUE, playerID, position)
                            );
                            if (tmpVal >= 50)
                            {
                                goto RepFor50_2;
        ...
        float lastChipH = 0;
        float lastChipX = 0;
        public IEnumerator createAndMoveChip(
            float chipVal,
            GameObject chip,
            int playerID,
            Transform position
        ...
        {
            GetComponent<BBSoundController>()
                .playClip(GetComponent<BBSoundController>().chipHandle);

            GameObject newChip =
                Instantiate(chip, playersChipStartingPoint[playerID].position, Quaternion.identity)
                as GameObject;
            ...
            if (lastChipH == 0)
                lastChipH = tmpT.position.y;
            else
                lastChipH += 0.01f;
            if (lastChipX == 0)
                lastChipX = tmpT.position.x;
            else
                lastChipX = 0.02f;

            tmpT.position = new Vector3(
                tmpT.position.x /* + lastChipX UnityEngine.Random.Range(0.03f,0.04f)*/
                ,
                lastChipH,
                tmpT.position.z
            );
        ...
        public IEnumerator createAndMoveChipFromTo(
            float chipVal,
            GameObject chip,
            Vector3 fromV3,
            Vector3 toV3
