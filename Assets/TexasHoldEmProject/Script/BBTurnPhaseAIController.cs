...
namespace BLabTexasHoldEmProject
{
    public class BBTurnPhaseAIController : MonoBehaviour
    {
        ...
        public IEnumerator startExecuteTurnResult()
        {
            ...
            GetComponent<BBGuiInterface>().TextGameRAISEValue.text =
                "Round Raise Value : "
                + BBStaticData.getMoneyValue(BBGC._BBGlobalDefinitions.bigBlindValue);
            ...
            {
                if (BBStaticData.debugTurnController)
                    Debug.Log(
                        playerCounter
                            + ")+++++++++++++++++++ [BBTurnPhaseAIController] playersStateIsOutDuringOpenGame : "
                ...
                if (
                    (BBGC.playerDataList[playerCounter].isOutOfGame == false)
                    && (
                ...
                )
                {
                    ...
                    if (playerCounter == BBGC._BBGlobalDefinitions.localPlayer)
                    {
                        ...
                        if (BBStaticData.debugTurnController)
                            Debug.Log(
                                "--------startExecuteTurnResult------------***DONE MANUAL***------------------->> startExecuteTurnResult playerToExecute : "
                                    + playerCounter
                            );
                    }
                    else
                    {
                        BBGC.playerDataList[playerCounter].coeffCardsValOnFlopPhase =
                            getCardsPointCoefficientOnTurnPhase(playerCounter, out maxCardVal);
                ...
                else
                {
                    if (BBStaticData.debugTurnController)
                        Debug.Log(
                            "ZZZZZZZZzzzzzzzzzzzzzzzzzzzzzz--------isOutOfGame------------***startExecuteTurnResult***------------------->> startExecuteTurnResult playerToExecute : "
                                + playerCounter
                        );
                }
            ...
            if (BBStaticData.debugTurnController)
                Debug.Log(
                    "ZZZZZZZZzzzzzzzzzzzzzzzzzzzzzz-- ready to ------------***start executeFinishTurnRound ***------------------->> executeFinishTurnRound playerToExecute : "
                        + playerCounter
                );
        }
        ...
        IEnumerator executeFinishTurnRound()
        {
            BBGC._BBGlobalDefinitions.gamePhaseDetail =
                BBGlobalDefinitions.GamePhaseDetail.ClosingTurn;
            GetComponent<BBGuiInterface>().TextGamePhase.text =
                "Phase : " + BBGC._BBGlobalDefinitions.gamePhaseDetail;
                    ...
                    else
                    {
                        if (BBGC.allInInAction)
                        {
                            ...
                            yield return StartCoroutine(
                                executeAIOnFinishTurnAction(playerCounter, 0)
                            );
                        }
                        else
                        {
                            ...
                            GetComponent<BBGuiInterface>().setGreenOnPlayer(playerCounter);
                            yield return StartCoroutine(
                                executeAIOnFinishTurnAction(playerCounter, tmpToCover)
                            );
                        }
                    }
                }
        ...
        int getCardsPointCoefficientOnTurnPhase(int playerID, out int maxCardValue)
        {
            ...
            if (BBStaticData.debugTurnController)
                Debug.Log(
                    "getCardsPointCoefficientOnRiverPhase--> retVal : "
                        + retVal
                        + " maxCardValue : "
                        + maxCardValue
                        + " playerID : "
        ...
        public IEnumerator executeTurnRaise(int playerID)
        {
            if (isMultiplayer)
            {
                ...
                if (BBStaticData.debugTurnController)
                    Debug.Log(
                        "executeTurnRaise Player id : "
                            + playerID
                            + " :BBPlayerAIController------> executeTurnRaise -----> raiseBetVal : "
                            + raiseBetVal
                    );
