...
namespace BLabTexasHoldEmProject
{
    public class BBGameController : MonoBehaviour
    {
        ...
        public bool checkIfLocalplayerWonBeforeShowDown()
        {
            ...
            {
                if (_BBGlobalDefinitions.localPlayer != x)
                {
                    if (_BBGlobalDefinitions.playersStateIsOutDuringOpenGame[x] == true)
                        outCounter++;
                }
            }
            if (outCounter > 8)
            {
                executeLocalPlayerWon(
                    playerDataList[_BBGlobalDefinitions.localPlayer].currentPlayerTotalMoney
                        + _BBGlobalDefinitions.moneyOnTable,
                    true
                );
                return true;
            }
            ...
            {
                if (playerDataList[x].isOutOfGame)
                    tmpOut++;
            }
            if (tmpOut > 8)
            {
        ...
        public void executeLocalPlayerWon(float localPlayerWonValue, bool wonCoseAlone)
        {
            ...
            GetComponent<BBShowDownResultController>().PanelLocalPlayerWon.transform
                .Find("TextWinnerMoneyWon")
                .GetComponent<Text>().text = BBStaticData.getMoneyValue(localPlayerWonValue);
            GameObject bng = GameObject.Find("ButtonNewGame");
            if (bng != null)
                bng.SetActive(false);
#if USE_GAME_CENTER
            GetComponent<BBGameCenterController>()
                .ReportScore((long)localPlayerWonValue, "grp.maxHandvalue");
#endif
        ...
        IEnumerator Start()
        {
            GetComponent<BBSoundController>()
                .playClip(GetComponent<BBSoundController>().cardsShuffle);
        ...
        bool checkForPlayersMoneyBeforeStart()
        {
            ...
            {
                if (
                    _BBGlobalDefinitions.playersCashDuringOpenGame[x]
                    < _BBGlobalDefinitions.limitedHight * BBStaticData.cardsHandProgressive
                )
                {
        ...
        bool setGameBeforeStart()
        {
            ...
            {
                if (
                    _BBGlobalDefinitions.playersStateIsOutDuringOpenGame[tmpSmallBPlayerID] == false
                )
                {
                ...
                else
                {
                    ...
                    if (
                        _BBGlobalDefinitions.playersStateIsOutDuringOpenGame[tmpSmallBPlayerID]
                        == false
                    )
                    {
                        gotSmallBlindPlayer = true;
            ...
            if (!gotSmallBlindPlayer)
                return false;

            int startFindBigBlind = tmpSmallBPlayerID + 1;
            if (startFindBigBlind == 10)
                startFindBigBlind = 0;

            Debug.Log(
                "firstLastPlayerToTalkOnTable ---------------> startFindBigBlind : "
                    + startFindBigBlind
                    + " tmpSmallBPlayerID : "
                    + tmpSmallBPlayerID
            );
            ...
            for (int x = 0; x < playerDataList.Count; x++)
            {
                Debug.Log(
                    "firstLastPlayerToTalkOnTable ---------------> startFindBigBlind : "
                        + startFindBigBlind
                        + " : "
                        + _BBGlobalDefinitions.playersStateIsOutDuringOpenGame[startFindBigBlind]
                );

                if (
                    _BBGlobalDefinitions.playersStateIsOutDuringOpenGame[startFindBigBlind] == false
                )
                {
            ...
            _BBGlobalDefinitions.firstLastPlayerToTalkOnTable = new Vector2(
                (float)first,
                _BBGlobalDefinitions.currentActivedealer
            );

            _BBGlobalDefinitions.smallBlindPlayerId = tmpSmallBPlayerID;
            _BBGlobalDefinitions.bigBlindPlayerId = startFindBigBlind;

            if (gotBigBlindPlayer)
                return true;
        ...
        IEnumerator startGame()
        {
            ...
            if (!setGameBeforeStart())
            {
                ...
                Debug.LogError(
                    " setGameBeforeStart XXXXXXXXXXXXXXXXXXXXXX firstLastPlayerToTalkOnTable ---------show message not players to play------> : "
                );
                yield break;
            }
            ...
            _BBGuiInterface.TextGamePhaseInfo.text =
                playerDataList[_BBGlobalDefinitions.currentActivePlayer].playerName
                + " : "
                + BBStaticData.phaseMessageInfoMakeYourSmallBlindBet;
            ...
            switch (_go.name)
            {
                    ...
                    switch (_BBGlobalDefinitions.gamePhaseDetail)
                    {
                        case BBGlobalDefinitions.GamePhaseDetail.FirstBettingRound:
                            yield return StartCoroutine(
                                GetComponent<BBPreFlopPhaseAIController>()
                                    .executeCall(_BBGlobalDefinitions.playerToTalk)
                            );
                            break;
                        ...
                        case BBGlobalDefinitions.GamePhaseDetail.Flop:
                            yield return StartCoroutine(
                                GetComponent<BBFlopPhaseAIController>()
                                    .executeFlopCall(_BBGlobalDefinitions.localPlayer)
                            );
                            break;
                        ...
                        case BBGlobalDefinitions.GamePhaseDetail.Turn:
                            yield return StartCoroutine(
                                GetComponent<BBTurnPhaseAIController>()
                                    .executeTurnCall(_BBGlobalDefinitions.localPlayer)
                            );
                            break;
                        ...
                        case BBGlobalDefinitions.GamePhaseDetail.River:
                            yield return StartCoroutine(
                                GetComponent<BBRiverPhaseAIController>()
                                    .executeRiverCall(_BBGlobalDefinitions.localPlayer)
                            );
                            break;
                ...
                case "buttonGameFOLD":
                    switch (_BBGlobalDefinitions.gamePhaseDetail)
                    {
                        case BBGlobalDefinitions.GamePhaseDetail.FirstBettingRound:
                            yield return StartCoroutine(
                                GetComponent<BBPreFlopPhaseAIController>()
                                    .executeFold(_BBGlobalDefinitions.localPlayer)
                            );
                            break;
                        case BBGlobalDefinitions.GamePhaseDetail.ClosingPreFlop:
                            yield return StartCoroutine(
                        ...
                        case BBGlobalDefinitions.GamePhaseDetail.Flop:
                            yield return StartCoroutine(
                                GetComponent<BBFlopPhaseAIController>()
                                    .executeFlopFold(_BBGlobalDefinitions.localPlayer)
                            );
                            break;
                        case BBGlobalDefinitions.GamePhaseDetail.ClosingFlop:
                            yield return StartCoroutine(
                        ...
                        case BBGlobalDefinitions.GamePhaseDetail.Turn:
                            yield return StartCoroutine(
                                GetComponent<BBTurnPhaseAIController>()
                                    .executeTurnFold(_BBGlobalDefinitions.localPlayer)
                            );
                            break;
                        case BBGlobalDefinitions.GamePhaseDetail.ClosingTurn:
                            yield return StartCoroutine(
                        ...
                        case BBGlobalDefinitions.GamePhaseDetail.River:
                            yield return StartCoroutine(
                                GetComponent<BBRiverPhaseAIController>()
                                    .executeRiverFold(_BBGlobalDefinitions.localPlayer)
                            );
                            break;
                        case BBGlobalDefinitions.GamePhaseDetail.ClosingRiver:
                            yield return StartCoroutine(
                ...
                case "buttonGameRAISE":
                    switch (_BBGlobalDefinitions.gamePhaseDetail)
                    {
                        case BBGlobalDefinitions.GamePhaseDetail.FirstBettingRound:
                            yield return StartCoroutine(
                                GetComponent<BBPreFlopPhaseAIController>()
                                    .executeRaise(_BBGlobalDefinitions.localPlayer)
                            );
                            break;
                        case BBGlobalDefinitions.GamePhaseDetail.ClosingPreFlop:
                            yield return StartCoroutine(
                        ...
                        case BBGlobalDefinitions.GamePhaseDetail.Flop:
                            yield return StartCoroutine(
                                GetComponent<BBFlopPhaseAIController>()
                                    .executeFlopRaise(_BBGlobalDefinitions.localPlayer)
                            );
                            break;
                        case BBGlobalDefinitions.GamePhaseDetail.ClosingFlop:
                            break;
                        case BBGlobalDefinitions.GamePhaseDetail.Turn:
                            yield return StartCoroutine(
                                GetComponent<BBTurnPhaseAIController>()
                                    .executeTurnRaise(_BBGlobalDefinitions.localPlayer)
                            );
                            break;
                        case BBGlobalDefinitions.GamePhaseDetail.ClosingTurn:
                            yield return StartCoroutine(
                        ...
                        case BBGlobalDefinitions.GamePhaseDetail.River:
                            yield return StartCoroutine(
                                GetComponent<BBRiverPhaseAIController>()
                                    .executeRiverRaise(_BBGlobalDefinitions.localPlayer)
                            );
                            break;
                        case BBGlobalDefinitions.GamePhaseDetail.ClosingRiver:
                            yield return StartCoroutine(
                ...
                case "buttonGameCHECK":
                    switch (_BBGlobalDefinitions.gamePhaseDetail)
                    {
                        ...
                        case BBGlobalDefinitions.GamePhaseDetail.Turn:
                            yield return StartCoroutine(
                                GetComponent<BBTurnPhaseAIController>()
                                    .executeTurnCheck(_BBGlobalDefinitions.localPlayer)
                            );
                            break;
                        case BBGlobalDefinitions.GamePhaseDetail.River:
                            yield return StartCoroutine(
                                GetComponent<BBRiverPhaseAIController>()
                                    .executeRiverCheck(_BBGlobalDefinitions.localPlayer)
                            );
                            break;
                    }
                    break;
                case "buttonGameALLIN":
                    switch (_BBGlobalDefinitions.gamePhaseDetail)
                    {
                        ...
                        case BBGlobalDefinitions.GamePhaseDetail.River:
                            yield return StartCoroutine(
                                GetComponent<BBAllInController>()
                                    .executeAllIN(_BBGlobalDefinitions.localPlayer)
                            );
                            break;
                    }
                    break;
        ...
        {
            Debug.Log(
                "*******************************[BBGameController][startExecute*** SHOW DOWN ***Phase]**************************"
            );
        ...
        {
            Debug.Log(
                "*******************************[BBGameController][startExecute*** RIVER ***Phase]**************************"
            );
            ...
            yield return StartCoroutine(
                giveOneCard(_BBGlobalDefinitions.currentActivedealer, cardsDiscardPosition, true)
            );
            ...
            yield return StartCoroutine(
                giveOneCard(
                    _BBGlobalDefinitions.currentActivedealer,
                    GameObject.Find("openCard_RIVER").transform,
                    false
                )
            );
        ...
        {
            Debug.Log(
                "*******************************[BBGameController][startExecute***TURN***Phase]**************************"
            );
            ...
            yield return StartCoroutine(
                giveOneCard(
                    _BBGlobalDefinitions.currentActivedealer,
                    GameObject.Find("openCard_TURN").transform,
                    false
                )
            );
        ...
        public IEnumerator startExecuteFlopPhase()
        {
            ...
            yield return StartCoroutine(
                giveOneCard(
                    _BBGlobalDefinitions.currentActivedealer,
                    GameObject.Find("openCard_1").transform,
                    false
                )
            );
            ...
            yield return StartCoroutine(
                giveOneCard(
                    _BBGlobalDefinitions.currentActivedealer,
                    GameObject.Find("openCard_2").transform,
                    false
                )
            );
            ...
            yield return StartCoroutine(
                giveOneCard(
                    _BBGlobalDefinitions.currentActivedealer,
                    GameObject.Find("openCard_3").transform,
                    false
                )
            );
            flopCardList[2] = currentGivenCard;
            Debug.Log(
                "*******************************[BBGameController][startExecuteFlopPhase]************************** cards : "
                    + flopCardList[0]
                    + " : "
                    + flopCardList[1]
                    + " : "
        ...
        void setNewGameData()
        {
            ...
            for (int x = 0; x < playerDataList.Count; x++)
            {
                currentPlayerscash[x] = playerDataList[x].currentPlayerTotalMoney;
            }
            _BBGlobalDefinitions.savePlayersCashDuringGame(currentPlayerscash);

            if (BBStaticData.currentActivePlayer == 9)
                BBStaticData.currentActivePlayer = 0;
            else
                BBStaticData.currentActivePlayer++;

            if (BBStaticData.currentActivedealer == 9)
                BBStaticData.currentActivedealer = 0;
            else
                BBStaticData.currentActivedealer++;
		...
		BLab.GetCoinsSystem.UnityAdvertisingController.ShowAdPlacement( BLab.GetCoinsSystem.UnityAdvertisingController.zoneIdVideo);
                ...
                case "ButtonGetBestCards":
                    List<BBAllInController.PlayersCValue> cardsV = GetComponent<BBAllInController>()
                        .getCardsResultDuringGameHand();
                    foreach (BBAllInController.PlayersCValue cv in cardsV)
                    {
                        Debug.Log(
                            "Get Best Five List PlayersCValue -> : "
                                + " pos : "
                                + cv.playerPosID
                                + " : "
                                + cv.bestFive[0]
        ...
        void gotBetButton(GameObject _go)
        {
            ...
            int currentActivePlayer =
                GetComponent<BBGameController>()._BBGlobalDefinitions.currentActivePlayer;
            ...
            StartCoroutine(
                GetComponent<BBMoveingObjectsController>()
                    .moveChip(
                        valToSet,
                        currentActivePlayer,
                        GetComponent<BBMoveingObjectsController>().playersChipEndingPoint[
                            currentActivePlayer
                        ]
                    )
            );
        ...
        public void setNextPlayerActive()
        {
            if (
                _BBGlobalDefinitions.gamePhaseDetail
                == BBGlobalDefinitions.GamePhaseDetail.SmallBlind
            )
            {
                ...
                _BBGuiInterface.TextGamePhaseInfo.text =
                    playerDataList[_BBGlobalDefinitions.currentActivePlayer].playerName
                    + " : "
                    + BBStaticData.phaseMessageInfoMakeYourBigBlindBet;
                _BBGuiInterface.activatePlayer();
        ...
        public void dealerStartGiveCards()
        {
            ...
            _BBGuiInterface.TextGamePhase.text =
                "Phase : " + _BBGlobalDefinitions.gamePhaseDetail.ToString();
            _BBGuiInterface.TextGamePhaseInfo.text =
                BBStaticData.phaseMessageInfoDealerGiveingCards;
            InitializeDeck();
        ...
        IEnumerator giveOneCard(int startPosPlayerId, Transform destPos, bool covered)
        {
            ...
            cartToGive.transform.position =
                GetComponent<BBMoveingObjectsController>().playersChipStartingPoint[
                    startPosPlayerId
                ].position;
            yield return new WaitForEndOfFrame();
            yield return StartCoroutine(
                GetComponent<BBMoveingObjectsController>().moveCard(cartToGive, destPos)
            );
        ...
        IEnumerator startGiveCards()
        {
            ...
            for (int x = 0; x < (playerDataList.Count * 2); x++)
            {
                ...
                GetComponent<BBSoundController>()
                    .playClip(GetComponent<BBSoundController>().pickCard);
                yield return new WaitForEndOfFrame();
                yield return StartCoroutine(
                    GetComponent<BBMoveingObjectsController>()
                        .moveCard(cartToGive, shuffledCardsList[x].transform, posVal)
                );
                Jump:
                yield return new WaitForEndOfFrame();
                lastGivenCardIdx = x;
        ...
        void executeFirstBettingRound()
        {
            ...
            StartCoroutine(
                GetComponent<BBPreFlopPhaseAIController>()
                    .executePreFlopFirstBetRound(_BBGlobalDefinitions.playerToTalk)
            );
        }
        ...
        Transform[] getGiveCardsList(int startPos)
        {
            ...
            for (int x = 0; x < tmpRes.Length; x++)
            {
                if (counter == 1)
                {
                    tmpRes[x] = playersCardPositionsRoot.Find(
                        idx.ToString() + "_" + counter.ToString()
                    );
                    counter++;
                }
                else
