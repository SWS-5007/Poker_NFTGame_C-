    ...
    public class BBFlopPhaseAIController : MonoBehaviour
    {
        ...
        public IEnumerator executeClosingFlopAllighMoneyValues(int underGunPlayer)
        {
            ...
            if (BBStaticData.debugPreFlopController)
                Debug.Log(
                    "[executeClosingFlopAllighMoneyValues][executeClosingFlopAllighMoneyValues] underGunPlayer : "
                        + underGunPlayer
                        + " phase : "
                        + BBGC_M._BBGlobalDefinitions.gamePhaseDetail
                        + " isLocalPlayer : "
                        + BBGC_M.playerDataList[playerCounter].isLocalPlayer
                        + " name : "
                        + BBGC_M.playerDataList[playerCounter].playerName
                        + " net player name : "
            ...
            if (BBGC_M.playerDataList[playerCounter].playerName == PhotonNetwork.player.name)
            {
                ...
                BBGC_M.UpadatePlayerGameState(
                    playerCounter,
                    BBGlobalDefinitions.GamePhaseDetail.ClosingFlop
                );
            ...
            else
            {
                ...
                for (int x = 0; x < BBGC.playerDataList.Count; x++)
                {
                    ...
                    )
                    {
                        ...
                        {
                            yield return StartCoroutine(executeManualOnFlopAction(playerCounter));
                            if (BBStaticData.debugFlopController)
                                Debug.Log(
                                    "--------startExecuteFlopResult------------***DONE MANUAL***------------------->> executeFlopResult playerToExecute : "
                                        + playerCounter
                                );
                        }
                        else
                        {
                            BBGC.playerDataList[playerCounter].coeffCardsValOnFlopPhase =
                                getCardsPointCoefficientOnFlopPhase(playerCounter, out maxCardVal);
                    ...
                    else
                    {
                        if (BBStaticData.debugFlopController)
                            Debug.Log(
                                "--------isOutOfGame------------***startExecuteFlopResult***------------------->> executeFlopResult playerToExecute : "
                                    + playerCounter
                            );
                    }
        ...
        IEnumerator executeFinishFlopRound()
        {
            ...
            for (int x = 0; x < BBGC.playerDataList.Count; x++)
            {
                ...
                )
                {
                    ...
                    if (playerCounter == BBGC._BBGlobalDefinitions.localPlayer)
                    {
                        ...
                        if (BBStaticData.debugFlopController)
                            Debug.Log(
                                "---------executeFinishFlopRound-----------***DONE MANUAL***------------------->> executeFlopResult playerToExecute : "
                                    + playerCounter
                                    + " x : "
                                    + x
                            );
                    ...
                    else
                    {
                        ...
                        GetComponent<BBGuiInterface>()
                            .setPlayerMoneyValue(tmpToCover, playerCounter);
                        GetComponent<BBGuiInterface>().setGreenOnPlayer(playerCounter);
                        yield return StartCoroutine(
                            executeAIOnFinishFlopAction(playerCounter, tmpToCover)
                        );
                    }
                }
            ...
            if (BBStaticData.debugFlopController)
                Debug.Log("------------executeFinishFlopRound  ***DONE*** ");

            if (BBGC.checkIfLocalplayerWonBeforeShowDown())
                yield break;
        ...
        int getCardsPointCoefficientOnFlopPhase(int playerID, out int maxCardValue)
        {
            ...
            BBPlayerCardsResultValueController.CardsValues retVal =
                GetComponent<BBPlayerCardsResultValueController>()
                    .getCardsResult(playerID, out maxCardValue);

            if (BBStaticData.debugFlopController)
                Debug.Log(
                    "getCardsPointCoefficientOnFlopPhase--> retVal : "
                        + retVal
                        + " maxCardValue : "
                        + maxCardValue
                        + " playerID : "
            ...
            if (BBGC.allInInAction && (playerID != BBGC.allInInActionForPlayer))
            {
                ...
                if (
                    GetComponent<BBAllInController>()
                        .checkIfCanAllInForAIPlayers(
                            playerID,
                            BBGC.playerDataList[playerID].currentPlayerTotalMoney,
                            BBGC.allInInActionValue
                        )
                ) { }
                else
                {
            ...
            StartCoroutine(
                GetComponent<BBMoveingObjectsController>()
                    .moveChip(
                        valToCover,
                        playerID,
                        GetComponent<BBMoveingObjectsController>().playersChipEndingPoint[playerID]
                    )
            );
        ...
        IEnumerator executeAIOnFlopAction(int playerID)
        {
            ...
            {
                if (
                    GetComponent<BBTestSimulationController>().simulateAiPhasePlayers_Flop[playerID]
                    == BBGlobalDefinitions.GamePhaseDetail.Flop
                )
                {
                    switch (
                        GetComponent<BBTestSimulationController>().simulateAiCommandPlayers_Flop[
                            playerID
                        ]
                    ) {
                        case BBTestSimulationController.TestSimulateAiCommand.Raise:
                            action = "Raise";
                            break;
                        case BBTestSimulationController.TestSimulateAiCommand.Check:
                            action = "Check";
                            break;
                    }
                }
            ...
            if (BBStaticData.debugFlopController)
                Debug.Log(
                    "Player id : "
                        + playerID
                        + " :BBPlayerAIController------> executeOnFlopAction -----> cardValCoeff : "
                        + cardValCoeff
                        + " action : "
                        + action
                        + " cardMaxVal : "
        ...
        public IEnumerator executeFlopFold(int playerID)
        {
            ...
            {
#if USE_PHOTON_BB
                if (BBGC_M.playerDataList[playerID].transform_card_1 == null)
                    yield break;
            ...
            {
                if (BBGC.playerDataList[playerID].transform_card_1 == null)
                    yield break;
        ...
        public IEnumerator executeFlopRaise(int playerID)
        {
            if (isMultiplayer)
            {
                ...
                GetComponent<BBGuiInterfaceMultiplayer>()
                    .setPlayerMoneyValue(raiseBetVal, playerID);

                if (BBStaticData.debugFlopController)
                    Debug.Log(
                        "executeFlopRaise Player id : "
                            + playerID
                            + " :BBPlayerAIController------> executeOnFlopAction -----> raiseBetVal : "
                            + raiseBetVal
                    );
        ...
        public IEnumerator executeClosingFlopCall(int playerID)
        {
            ...
            Debug.Log(
                "**********ie_gotGameActionButton**********************CONTROLLO CRASH*****7************ClosingMoneyAllign*****************"
            );

            if (BBGC_M.playerDataList[playerID].playerName == PhotonNetwork.player.name)
            {
                Debug.Log(
                    "**********ie_gotGameActionButton**********************CONTROLLO CRASH*****8************ClosingMoneyAllign*****************"
                );

                if (BBGC_M.playerDataList[playerID].currentMoneyOnTable < maxPlayerVal)
                {
                    float toAdd =
                        maxPlayerVal - BBGC_M.playerDataList[playerID].currentMoneyOnTable;
                    BBGC_M.UpadatePlayerGameState(
                        playerID,
                        BBGlobalDefinitions.GamePhaseDetail.ClosingMoneyAllign
                    );
                    ...
                    if (isMultiplayer)
                    {
                        GetComponent<PhotonView>()
                            .RPC("RPC_moveChip", PhotonTargets.All, toAdd, playerID);
                    }
                    else
                    {
                        yield return StartCoroutine(
        ...
        public IEnumerator executeFlopCall(int playerID)
        {
            if (isMultiplayer)
            {
                ...
                if (BBStaticData.debugFlopController)
                    Debug.Log("executeFlopCall--2-- player : " + playerID + " toBet : " + toBet);
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (BBGC._BBGlobalDefinitions.gameType == BBGlobalDefinitions.GameType.NoLimit)
                    {
                        ...
                        for (int x = 0; x < BBGC.playerDataList.Count; x++)
                        {
                            if (BBGC.playerDataList[x].underAllin == true)
                                allInCovered = true;
                        }
                        if (!allInCovered)
                            GetComponent<BBAllInController>().cleanAllInRequest();
                    }
                }
            }
        }
