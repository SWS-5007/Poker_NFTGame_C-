    ...
    public class BBRiverPhaseAIController : MonoBehaviour
    {
            ...
            for (int x = 0; x < BBGC.playerDataList.Count; x++)
            {
                ...
                )
                {
                    ...
                    if (playerCounter == BBGC._BBGlobalDefinitions.localPlayer)
                    {
                        ...
                        if (BBStaticData.debugRiverController)
                            Debug.Log(
                                "--------startExecuteRiverResult------------***DONE MANUAL***------------------->> startExecuteRiverResult playerToExecute : "
                                    + playerCounter
                            );
                    }
                    else
                    {
                        BBGC.playerDataList[playerCounter].coeffCardsValOnFlopPhase =
                            getCardsPointCoefficientOnRiverPhase(playerCounter, out maxCardVal);
                ...
                else
                {
                    if (BBStaticData.debugRiverController)
                        Debug.Log(
                            "ZZZZZZZZzzzzzzzzzzzzzzzzzzzzzz--------isOutOfGame------------***startExecuteRiverResult***------------------->> startExecuteRiverResult playerToExecute : "
                                + playerCounter
                        );
                }
        ...
        IEnumerator executeManualOnRiverAction(int playerID)
        {
            ...
            )
            {
                ...
                BBGC._BBGlobalDefinitions.gamePhaseDetail
                == BBGlobalDefinitions.GamePhaseDetail.ClosingRiver
            )
            {
        ...
        IEnumerator executeAIOnRiverAction(int playerID)
        {
            ...
            if (BBStaticData.debugRiverController)
                Debug.Log(
                    "BBPlayerAIController------> executeAIOnTurnAction -----> cardValCoeff : "
                        + cardValCoeff
                        + " action : "
                        + action
                        + " cardMaxVal : "
        ...
        IEnumerator executeFinishRiverRound()
        {
                ...
                )
                {
                    ...
                    else
                    {
                        ...
                        yield return StartCoroutine(
                            executeAIOnFinishRiverAction(playerCounter, tmpToCover)
                        );
                    }
                }
            ...
            Debug.Log(
                "-------------------------------->> executeFinishRiverRound playerToExecute : "
                    + playerCounter
            );
        ...
        public IEnumerator executeRiverRaise(int playerID)
        {
            if (isMultiplayer)
            {
                ...
                if (BBStaticData.debugTurnController)
                    Debug.Log(
                        "executeRiverRaise Player id : "
                            + playerID
                            + " :BBPlayerAIController------> executeRiverRaise -----> raiseBetVal : "
                            + raiseBetVal
                    );
